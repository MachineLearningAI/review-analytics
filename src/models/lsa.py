...
log = math.log
with open('../data/reviews.json', encoding='utf-8') as reviews_file:
...
log_n = log(len(doc_matrix[0]))
i = 0
for row in doc_matrix:
    print(i)
    i = i + 1
    gf_i = np.sum(row)
    weighted_sum = sum( ((elem/gf_i) * log(elem/gf_i)) for elem in row if elem > 0)
    g_i = (weighted_sum / log_n) + 1
    for index, elem in enumerate(row):
        row[index] = math.log(elem + 1) * g_i

lsa_transformer = TruncatedSVD(n_components = 100)
X = lsa_transformer.fit_transform(doc_matrix)
# clusters = KMeans(n_clusters=10).fit(svd)
